...
def teacher_or_student():  # выбор учитель или ученик при регистрации
    ...
    key_back = types.InlineKeyboardButton(text=EmojiAlphabet.back, callback_data='back_registration')
    ...
    if page == 1:  # передали 1-ю страницу
        ...
        if length <= 4:
            ...
            if n == 0:  # делиться с нулевым остатком только число 4, значит 4 кнопки
                ...
                key1 = types.InlineKeyboardButton(text=group_list[page * 4 - 4], callback_data=f'reg_{group_list[page * 4 - 4]}')
                key2 = types.InlineKeyboardButton(text=group_list[page * 4 - 3], callback_data=f'reg_{group_list[page * 4 - 3]}')
                key3 = types.InlineKeyboardButton(text=group_list[page * 4 - 2], callback_data=f'reg_{group_list[page * 4 - 2]}')
                key4 = types.InlineKeyboardButton(text=group_list[page * 4 - 1], callback_data=f'reg_{group_list[page * 4 - 1]}')
