...
def change_homeworks(department):
    ...
    id_hw = [item for item in cursor.fetchall()][0]
    conn.commit()
    ...
    for i in list(range(n)):
        ...
        title = [item for item in cursor.fetchall()][0]
        conn.commit()

        data = 'prepod_change_{}'.format(id_hw[i - 1])
        key = types.InlineKeyboardButton(text=u"\U0001F4CD" + title,
                                         callback_data=data)
        keyboard.add(key)
        n -= 1
...
def create_text(title, text, deadline_d, deadline_t, file):  # после ввода текста
    msg = '\U0001F4DA<code>Домашнее задание:</code>\n\n\U0001F4CD<b>Заголовок:</b> "{}"\n\n\U0000270F<b>Текст домашнего задания:</b>\n{}\n\n\U000023F0<b>Дедлайн</b> - Дата: <i>{}</i>\n       Время: <i>{}</i>\n\n\U0001F4CE<b>Файл</b> - <i>{}</i>'.format(
        title, text, deadline_d, deadline_t, file)

    keyboard = types.InlineKeyboardMarkup()
    key_1 = types.InlineKeyboardButton(text='Дедлайн \U000023F0', callback_data='deadline')
    key_2 = types.InlineKeyboardButton(text='Файл' + u"\U0001F4C4", callback_data='plus_file')
    key_3 = types.InlineKeyboardButton(text='Подтвердить \U00002705', callback_data='confirm')
    key_back = types.InlineKeyboardButton(text=EmojiAlphabet.back, callback_data='back_hw_teacher')
