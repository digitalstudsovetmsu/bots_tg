import re
from SoftUbot import settings

def validFIO(text):

    settings.cursor.execute("SELECT COUNT (*) FROM all_in_one WHERE username = '{}'".format(text))
    ...
    if not 1 < len(text.split(' ')) < 4:
        msg = 'Должно быть всего 2 или 3 слова'
        return msg
...
def validDeadline(text):
    ...
    if not re.findall(r'\d{2}\.\d{2}', text) != []:
        ...
        if 0 < int(day_d) < 32 and 0 < int(month_d) < 12:
            msg = 'Всё отлично!'
        else:
